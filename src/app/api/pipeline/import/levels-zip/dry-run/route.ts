...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await importLevelsZip({ fileBuffer: buffer, dryRun: true });
    await prisma.importJob.update({ where: { id: job.id }, data: { status: "completed", summaryJson: JSON.stringify(data), validationJson: JSON.stringify(data.items), fileName: file.name } });
