...
async function getContent(request: Request) {
  const body = await request.json();
  return String(body.fileContent ?? "");
}

export async function POST(request: Request) {
  const job = await prisma.importJob.create({ data: { type: "level_json", status: "running" } });
  try {
    const fileContent = await getContent(request);
    const data = await importLevelJson({ fileContent, dryRun: true });
