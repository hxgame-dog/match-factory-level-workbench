...
export async function POST(request: Request) {
  try {
    ...
    const data = await buildProductionPackage({ ...payload, dryRun: false });
    return NextResponse.json({ success: true, data });
  } catch (error) {
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : "构建失败" }, { status: 400 });
  }
}
