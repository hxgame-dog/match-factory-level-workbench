export const zh = {
  ...
  pages: {
    ...
    itemGenerator: {
      title: "AI 道具表生成器",
      description: "基于道具库与主题，用 Gemini 生成可编辑道具集",
      configTitle: "生成配置",
      configDesc: "填写主题与数量约束后生成；生成结果可在下方预览并编辑",
      previewTitle: "道具列表预览",
      previewEmptyTitle: "尚未生成道具表",
      previewEmptyDesc: "完成左侧配置后点击「生成道具表」，结果将显示在此处",
      catalogTitle: "道具库参考",
      catalogDesc: "当前道具库统计，供 AI 挑选候选道具时参考",
      historyTitle: "历史生成记录",
      fields: {
        setName: { label: "道具集名称", hint: "保存到数据库时使用的名称" },
        theme: { label: "关卡主题", hint: "如「早餐」「运动器材」，决定道具风格与分类" },
        totalItemCount: { label: "道具总数量", hint: "本套道具表期望的总条数（含目标物与干扰物）" },
        targetTypeCount: { label: "目标物种类数", hint: "玩家需要收集的目标物类型数量" },
        targetCountEach: { label: "每种目标物数量", hint: "每一类目标物在关卡中的件数" },
        distractorTypeCount: { label: "干扰物种类数", hint: "用于增加难度的非目标物类型数" },
        difficultyIntent: { label: "难度倾向", hint: "影响颜色/形状相似度与干扰强度" },
        constraints: { label: "额外约束", hint: "自由描述，如「少用红色」「避免圆形」" },
        useExistingCatalogOnly: { label: "仅使用道具库已有道具", hint: "开启后 AI 不新建道具，仅从已导入目录挑选" },
      },
      difficulty: {
        easy: "简单",
        normal: "普通",
