...
export function LevelBoardCell({ item, asset, selected, onClick }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative aspect-square rounded border p-1 text-left ${selected ? "border-blue-500 ring-2 ring-blue-200" : "border-gray-200"} ${item ? "bg-white" : "bg-gray-50"}`}
      ...
      {item ? (
        <>
          {asset?.imageUrl ? (
            ...
            <div className="flex h-full items-center justify-center text-xs text-gray-500">{item.name.slice(0, 1)}</div>
          )}
