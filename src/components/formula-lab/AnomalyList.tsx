...
export function AnomalyList({ anomalies }: { anomalies: DifficultyAnomaly[] }) {
  if (anomalies.length === 0) return <div className="text-sm text-gray-500">无异常关卡</div>;
  return (
    <div className="space-y-2">
      {anomalies.map((a, i) => (
        <div key={`${a.levelId}-${a.type}-${i}`} className="rounded-md border border-gray-200 p-2 text-sm">
